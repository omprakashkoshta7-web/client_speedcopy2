...
import { isNotFoundError, isRouteNotFoundError, wrapSuccess } from './api-response.utils';
...
class FinanceService {
  ...
  async getWalletBalance() {
    try {
      ...
      return wrapSuccess({ balance: wallet.balance || 0 });
    }
  }
  ...
  async getWalletOverview() {
    try {
      ...
      const bundle = this.extractWalletBundle(await this.getWalletBundle());
      return wrapSuccess(bundle?.overview || {});
    }
  }
  ...
  async getLedger(params?: { page?: number; limit?: number; type?: string }) {
    try {
      ...
      const bundle = this.extractWalletBundle(await this.getWalletBundle());
      return wrapSuccess(bundle?.ledger || {});
    }
  }
  ...
  async getTopupConfig() {
    try {
      ...
      return wrapSuccess(topupConfig);
    }
  }
