...
class GiftingService {
  ...
  private normalizeProduct<T extends { images?: string[]; thumbnail?: string; image?: string; imageUrl?: string; category?: GiftingCategory | null }>(product: T): T {
    return {
      ...product,
  ...
  private normalizeProductsResponse(response: ApiResponse<GiftingProductsResponse>): ApiResponse<GiftingProductsResponse> {
    return {
      ...response,
      data: {
        ...response.data,
  ...
  private normalizeHomeResponse(response: ApiResponse<GiftingHomeData>): ApiResponse<GiftingHomeData> {
    return {
      ...response,
      data: {
        ...response.data,
        ...
        customizable_products: (response.data?.customizable_products || []).map((product) => this.normalizeProduct(product)),
        premium_designs: (response.data?.premium_designs || []).map((product) => this.normalizeProduct(product)),
      },
    };
  }
  ...
  async getHome(): Promise<ApiResponse<GiftingHomeData>> {
    try {
      ...
      localStorage.setItem('gifting_home_data', JSON.stringify(normalized));
      return normalized;
    } catch (error: any) {
  ...
  async getCategories(): Promise<ApiResponse<GiftingCategory[]>> {
    try {
      ...
      // Cache categories
      localStorage.setItem('gifting_categories', JSON.stringify(normalized));
  ...
  async listProducts(query: GiftingListQuery = {}): Promise<ApiResponse<GiftingProductsResponse>> {
    try {
      ...
      localStorage.setItem(`gifting_products_${JSON.stringify(query)}`, JSON.stringify(normalized));
      return normalized;
    } catch (error: any) {
  ...
  async getProduct(identifier: string): Promise<ApiResponse<GiftingProductDetail>> {
    try {
      ...
      localStorage.setItem(`gifting_product_${identifier}`, JSON.stringify(normalized));
