...
import { normalizeImageList, normalizeImageUrl } from '../utils/image.utils';
...
class ShoppingService {
  private normalizeCategory<T extends { image?: string }>(category: T | null): T | null {
    return category ? { ...category, image: normalizeImageUrl(category.image) } : category;
  }

  private normalizeProduct<T extends { images?: string[]; thumbnail?: string; image?: string; imageUrl?: string; category?: ShoppingCategory | null }>(product: T): T {
    return {
      ...product,
      images: Array.isArray(product.images) ? normalizeImageList(product.images) : product.images,
      thumbnail: normalizeImageUrl(product.thumbnail),
      image: normalizeImageUrl(product.image),
      imageUrl: normalizeImageUrl(product.imageUrl),
      category: this.normalizeCategory(product.category || null),
    };
  }

  private normalizeProductsResponse(response: ApiResponse<ShoppingProductsResponse>): ApiResponse<ShoppingProductsResponse> {
    return {
      ...response,
      data: {
        ...response.data,
        products: (response.data?.products || []).map((product) => this.normalizeProduct(product)),
      },
    };
  }

  private normalizeHomeResponse(response: ApiResponse<ShoppingHomeData>): ApiResponse<ShoppingHomeData> {
    return {
      ...response,
      data: {
        ...response.data,
        banners: (response.data?.banners || []).map((banner) => ({
          ...banner,
          image: normalizeImageUrl(banner.image),
        })),
        categories: (response.data?.categories || []).map((category) => this.normalizeCategory(category)!),
        deal_of_day: response.data?.deal_of_day ? this.normalizeProduct(response.data.deal_of_day) : null,
        trending_products: (response.data?.trending_products || []).map((product) => this.normalizeProduct(product)),
        featured_products: (response.data?.featured_products || []).map((product) => this.normalizeProduct(product)),
      },
    };
  }
  ...
  async getHome(): Promise<ApiResponse<ShoppingHomeData>> {
    try {
      ...
      const normalized = this.normalizeHomeResponse(response.data);

      // Cache home data
      localStorage.setItem('shopping_home_data', JSON.stringify(normalized));
  ...
  async getCategories(): Promise<ApiResponse<ShoppingCategory[]>> {
    try {
      ...
      const normalized = {
        ...response.data,
        data: (response.data?.data || []).map((category) => this.normalizeCategory(category)!),
      };

      // Cache categories
      localStorage.setItem('shopping_categories', JSON.stringify(normalized));
  ...
  async listProducts(query: ShoppingListQuery = {}): Promise<ApiResponse<ShoppingProductsResponse>> {
    try {
      ...
      localStorage.setItem(`shopping_products_${JSON.stringify(query)}`, JSON.stringify(normalized));
      return normalized;
    } catch (error: any) {
  ...
  async getProduct(slugOrId: string): Promise<ApiResponse<ShoppingProductDetail>> {
    try {
      ...
      localStorage.setItem(`shopping_product_${slugOrId}`, JSON.stringify(normalized));
  ...
  async searchProducts(query: ShoppingSearchQuery): Promise<ApiResponse<ShoppingProductsResponse>> {
    try {
      ...
      return this.normalizeProductsResponse(response.data);
    } catch (error: any) {
  ...
  async getDealOfDay(): Promise<ApiResponse<DealOfDay | null>> {
    try {
      ...
      const normalized = {
        ...response.data,
        data: response.data.data ? this.normalizeProduct(response.data.data) : null,
      };

      // Cache deal
      if (normalized.data) {
        localStorage.setItem('shopping_deal_of_day', JSON.stringify(normalized));
      }
  ...
  async getTrendingProducts(): Promise<ApiResponse<ShoppingProductCard[]>> {
    try {
      ...
      localStorage.setItem('shopping_trending_products', JSON.stringify(normalized));
