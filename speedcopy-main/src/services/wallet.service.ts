...
// ── Wallet Order Payment Types ─────────────────────────────────────────────────

export interface WalletPayOrderResponse {
  success: boolean;
  message: string;
  data: {
    order: {
...
export interface WalletOrderPaymentError {
  success: false;
  message: string;
  code?: string;
  errors?: any;
...
class WalletService {
  ...
  /**
   * Main wallet checkout endpoint - pays for an order using wallet balance
   * POST /api/wallet/pay-order
   */
  async payOrderWithWallet(orderId: string): Promise<{ success: boolean; data?: WalletPayOrderResponse['data']; message?: string }> {
    try {
      const res = await apiClient.post(API_CONFIG.ENDPOINTS.FINANCE.WALLET_PAY_ORDER, { orderId });
      ...
      return {
        success: false,
        message: errorData?.message || err?.message || 'Failed to pay with wallet',
      };
    }
  }

  /**
   * Alternative endpoint: pay for order using order ID in URL
   * POST /api/orders/:id/pay-with-wallet
   */
  async payOrderWithWalletById(orderId: string): Promise<{ success: boolean; data?: WalletPayOrderResponse['data']; message?: string }> {
    try {
      const res = await apiClient.post(API_CONFIG.ENDPOINTS.FINANCE.ORDERS_PAY_WITH_WALLET(orderId));
      const response = res.data;
  ...
  /**
   * Compatibility endpoint: payments namespace
   * POST /api/payments/wallet/pay
   */
  async payWithWalletPayments(orderId: string): Promise<{ success: boolean; data?: WalletPayOrderResponse['data']; message?: string }> {
    try {
      const res = await apiClient.post(API_CONFIG.ENDPOINTS.FINANCE.PAYMENTS_WALLET_PAY, { orderId });
      const response = res.data;
  ...
  /**
   * Check wallet balance and return if sufficient for order amount
   */
  async hasSufficientBalance(orderTotal: number): Promise<{ sufficient: boolean; balance: number; required: number }> {
