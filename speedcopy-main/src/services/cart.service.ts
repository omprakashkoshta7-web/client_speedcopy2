...
const normalizeCouponResponse = (payload: any): ApiResponse<CouponResponse> => {
  const raw = payload?.data && typeof payload.data === 'object' ? payload.data : payload;
  const discountType = raw?.discountType === 'flat' ? 'fixed' : raw?.discountType;
  const discount = Number(raw?.discount) || 0;
  const finalTotal = Number(raw?.finalTotal ?? raw?.finalAmount) || 0;
  ...
  return {
    ...
    data: {
      ...
      discount,
      finalTotal,
      code: raw?.code || raw?.couponCode || '',
      finalAmount: Number(raw?.finalAmount ?? raw?.finalTotal) || finalTotal,
    },
  };
};
...
class CartService {
   ...
   * @param subtotal Optional subtotal for estimated discount calculation
   * @returns List of available coupons with estimated discounts
   */
  async getAvailableCoupons(
    flowType?: 'printing' | 'gifting' | 'shopping',
    subtotal?: number
  ): Promise<ApiResponse<{ coupons: AvailableCoupon[] }>> {
    const params: Record<string, string | number> = {};
    if (flowType) params.flowType = flowType;
    if (subtotal !== undefined) params.subtotal = subtotal;

    const response = await apiClient.get<ApiResponse<{ coupons: AvailableCoupon[] }>>(
      API_CONFIG.ENDPOINTS.CART.AVAILABLE_COUPONS,
      { params }
    );
    return response.data;
  }
