...
class BusinessPrintingService {
  ...
  async getBusinessPrintingTypes(): Promise<{ success: boolean; data: BusinessPrintType[] }> {
    try {
      ...
      return wrapSuccess(cached ? JSON.parse(cached) : []);
    }
  }
  ...
  async getBusinessProducts(params?: {
    ...
    try {
      ...
      if (body?.data?.products) {
        body.data.products = body.data.products.map((p: any) => ({
          ...p,
          images: Array.isArray(p.images) ? p.images.map((img: string) => normalizeImageUrl(img)).filter(Boolean) : p.images,
          thumbnail: normalizeImageUrl(p.thumbnail || p.images?.[0]),
          image: normalizeImageUrl(p.image),
          imageUrl: normalizeImageUrl(p.imageUrl),
        }));
      }
      console.log('✅ Business products response:', body);
      return body;
    } catch (error) {
  ...
  async getBusinessPrintConfig(configId: string): Promise<{ success: boolean; data: BusinessPrintConfig }> {
    try {
      ...
      if (cached) {
        return wrapSuccess(JSON.parse(cached));
      }
