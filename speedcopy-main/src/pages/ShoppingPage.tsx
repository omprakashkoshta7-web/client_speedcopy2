...
const getNumericPrice = (item: any): number | null => {
  const candidates = [
    item?.starting_from,
    item?.basePrice,
...
const getCategoryKeys = (category: any): string[] => {
  const rawKeys = [
...
const ShoppingPage: React.FC = () => {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      const [shoppingCatRes, shoppingProductsRes] = await Promise.all([
        productService.getShoppingCategories(),
        productService.getProductsByFlowType('shopping', { limit: 100 })
      ]);
      ...
      const shoppingProducts = shoppingProductsRes.data?.products || [];
      const categoryPriceMap = new Map<string, number>();

      shoppingProducts.forEach((product: any) => {
        const price = getNumericPrice(product);
        if (price === null) return;

        getCategoryKeys(product.category).forEach((key) => {
          const currentPrice = categoryPriceMap.get(key);
          if (currentPrice === undefined || price < currentPrice) {
            categoryPriceMap.set(key, price);
          }
        });
      });

      setCategories(categoriesData.map((category: any) => {
        const categoryPrice = getNumericPrice(category);
        const derivedPrice = getCategoryKeys(category)
          .map((key) => categoryPriceMap.get(key))
          .find((price) => price !== undefined);

        return {
          ...category,
          displayStartingPrice: categoryPrice ?? derivedPrice ?? null,
        };
      }));
      setTrendingProducts(shoppingProducts.slice(0, 4));
    } catch (err) {
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {loading ? (
              ...
              categories.map((cat: any) => {
                ...
                const price = formatPrice(cat.displayStartingPrice);
                return (
                  <div key={cat._id} onClick={() => navigate(`/products?flow=shopping&category=${encodeURIComponent(cat.name)}`)} className="bg-white rounded-2xl overflow-hidden cursor-pointer hover:shadow-md transition" style={{ border: '1px solid #f3f4f6' }}>
                    ...
                    <div className="p-4 bg-white">
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        <span className="font-bold text-gray-900 text-sm">{price || 'Price coming soon'}</span>
                      </div>
                    </div>
                  </div>
                );
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
            {loading ? (
              ...
              trendingProducts.map((p: any) => {
                const imageUrl = resolveThumbnail(p.images, p.thumbnail, p.image, p.imageUrl);
                const discountInfo = calculateDiscountInfo(p);
                const priceDisplay = formatPriceDisplay(discountInfo);
                const dealCountdown = getDealCountdown(p);
