...
const GiftingProductDetailPage: React.FC = () => {
  ...
  const normalizedImages = useMemo(() => {
    ...
    const resolved = Array.from(new Set(vals.map((value) => resolveImageUrl(value)).filter(Boolean)));
    return resolved.length ? resolved : [
      'https://images.unsplash.com/photo-1514228742587-6b1558fcca3d?w=900&q=80',
    ];
  }, [product]);

  const handleProductImageError = (source: string) => (event: React.SyntheticEvent<HTMLImageElement>) => {
  ...
  return (
    <div style={{ backgroundColor: '#f5f5f5', minHeight: '100vh' }}>
          ...
          <div>
            ...
            >
              <img
                ...
                onError={handleProductImageError(normalizedImages[Math.min(activeImg, normalizedImages.length - 1)])}
              />
            </div>
            ...
            {normalizedImages.length > 1 && (
              <div className="flex items-center gap-3 mt-4 overflow-x-auto pb-1">
                {normalizedImages.map((img, i) => (
                  ...
                  >
                    <img src={img} alt={`${productName} ${i + 1}`} className="w-full h-full object-cover" onError={handleProductImageError(img)} />
                  </button>
                ))}
              </div>
            )}
              ...
              <div className="flex-1">
                <p className="text-xs font-bold" style={{ color: '#1b5e20' }}>
                  Estimated Delivery: {(() => {
                    const now = new Date();
                    const cutoff = 14;
                    let days = now.getHours() >= cutoff ? 4 : 3;
                    let d = new Date(now);
                    let added = 0;
                    while (added < days) {
                ...
                <p className="text-xs mt-0.5" style={{ color: '#2e7d32' }}>
                  {new Date().getHours() >= 14 ? 'Order now · dispatches tomorrow' : 'Order now · dispatches today'} · Sundays excluded
                </p>
              </div>
            </div>
          </div>
      ...
      {/* Variant Selector Modal — shown when user clicks "Customise Now" */}
      {showVariantModal && (
