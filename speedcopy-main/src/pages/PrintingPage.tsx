...
const PrintTypeModal: React.FC<{ onClose: () => void; printTypes: any[] }> = ({ onClose, printTypes }) => {
  ...
  const getPrintConfigType = (pt: any) => {
    const rawType = String(pt.id || pt.slug || pt.name || pt.label || '').trim().toLowerCase();
    const normalized = rawType.replace(/\s+/g, '-').replace(/_/g, '-');
    const map: Record<string, string> = {
      standard: 'standard',
      'standard-printing': 'standard',
      soft: 'soft-binding',
        ...
        >
          {printTypes.map((pt: any) => (
            <button
              ...
              onClick={() => {
                onClose();
                navigate(`/print-config?type=${getPrintConfigType(pt)}`);
              }}
            >
...
const PrintingPage: React.FC = () => {
  ...
  const fetchPrintTypes = async () => {
    try {
      ...
      setPrintTypes([
        { id: 'standard', label: 'Standard Printing', desc: 'Perfect for reports & essays', icon: 'document' },
        { id: 'soft-binding', label: 'Soft Binding', desc: 'Clean professional look', icon: 'book' },
        { id: 'spiral-binding', label: 'Spiral Binding', desc: 'Durable & easy to flip', icon: 'spiral' },
        { id: 'thesis-binding', label: 'Thesis Binding', desc: 'Official university standard', icon: 'thesis' },
      ]);
    }
  };
