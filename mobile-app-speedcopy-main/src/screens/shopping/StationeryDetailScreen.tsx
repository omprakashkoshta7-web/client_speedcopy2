...
const FALLBACK_DESCRIPTION =
  'High-quality stationery essential, designed for everyday writing and creativity.';
const FALLBACK_FEATURES = [
  'Premium materials with refined finish',
...
export function StationeryDetailScreen() {
  ...
  const initialImage: ImageSourcePropType = passedImageUri ? { uri: passedImageUri } : IMG_NOTEBOOKS;
  ...
  useEffect(() => {
    ...
    productsApi.getShoppingProduct(productId)
      ...
      .catch(() => {
        // keep passed-in card data on API failure
      })
      .finally(() => {
  ...
  }, [addItem, imageUri, product, productId, quantity, selectedGalleryUri, stockState.inStock]);
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.thumbRow}>
          {(galleryImages.length ? galleryImages : [undefined, undefined, undefined, undefined]).map((thumbUri, i) => (
            <TouchableOpacity
              ...
              style={[
                styles.thumbCard,
                { borderColor: selectedThumb === i ? '#0F766E' : t.border },
                selectedThumb === i && styles.thumbCardActive,
              ]}
            >
