...
const PrintConfigPage: React.FC = () => {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      await orderService.addToCart({
        ...
        thumbnail: uploadedFiles[0]?.data || undefined,
        options: { 
          configId, 
          printType,
          colorMode,
          ...
          bindingType,
          coverPage,
          totalPages: uploadedFiles.reduce((sum, f) => sum + (f.pages || 1), 0),
        },
      });
      navigate('/cart');
    } catch (err: any) {
  ...
  const handleContinueToPay = async () => {
    ...
    try {
      ...
      if (isAuthenticated) {
        const configResponse = await productService.saveBusinessPrintConfig(buildConfigPayload());
        const apiConfigId = configResponse.data?._id || configResponse.data?.configId;
